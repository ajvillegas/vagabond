	...
	$( document ).ready( function() {
			...
			if ( numRepeaterItems > 0 ) {
				...
				if ( numRepeaterItems > 1 ) {
					...
					for ( i = 1; i < numRepeaterItems; ++i ) {
						ajvcustomizeAppendRow( $( this ), defaultValuesArray[i] );
					}
				}
			}
		} );
		...
		$( this ).find( '.sortable' ).sortable( {
			update: function( event, ui ) {
				ajvcustomizeGetAllInputs( $( this ).parent() );
			}
		} );
		...
		$( '.sortable' ).on( 'click', '.customize-control-sortable-repeater-delete', function( event ) {
			...
			if ( numItems > 1 ) {
				$( this ).parent().slideUp( 'fast', function() {
					...
					$( this ).remove();
					ajvcustomizeGetAllInputs( parentContainer );
				})
			} else {
				$( this ).parent().find( '.repeater-input' ).val( '' );
				ajvcustomizeGetAllInputs( $( this ).parent().parent().parent() );
			}
		} );
		...
		$( '.customize-control-sortable-repeater-add' ).click( function( event ) {
			event.preventDefault();
			ajvcustomizeAppendRow( $( this ).parent() );
			ajvcustomizeGetAllInputs( $( this ).parent() );
		} );
		...
		function ajvcustomizeGetAllInputs( $element ) {
	    ...
	    $( document.body ).on( 'click.ajvStarterOpenMediaManager', '.vagabond-custom-media-button', function( e ) {

	        // Prevent the default action from occuring.
	        e.preventDefault();

	        // Get our custom input field ID.
		    var target_input = $( this ).siblings( '.repeater-input' );
	        ...
	        media_frame = wp.media.frames.media_frame = wp.media( {
	            ...
	            className: 'media-frame vagabond-media-frame',
		...
		$( '.google-fonts-list' ).on( 'change', function() {
			...
			var indexes = $.map( bodyfontcontrol.ajvcustomizefontslist, function( obj, index ) {
			...
			$.each( bodyfontcontrol.ajvcustomizefontslist[index].variants, function( val, text ) {
			...
			$( this ).parent().parent().find( '.google-fonts-category' ).val( bodyfontcontrol.ajvcustomizefontslist[index].category );

			ajvcustomizeGetAllSelects( $( this ).parent().parent() );
		} );
		...
		function ajvcustomizeGetAllSelects( $element ) {
			var selectedFont = {
